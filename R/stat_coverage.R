...
#' df <- DataFrame(x = GRanges(c("chr1:100-200", "chr1:140-260", "chr2:50-100")))
#' ggplot(df, aes(x)) +
#'   stat_coverage()
...
StatCoverage <- ggproto(
  ...
  aesthetics = function(self) {
    ...
    if (is.null(self$optional_aes)) {
      ...
      optional_aes <- unlist(strsplit(self$optional_aes, "|", fixed = TRUE))
    }
  ...
  compute_group = function(data, scales, flipped_aes = FALSE, trim = FALSE,
    ...
    if (cov_range) {
      coverage <- compute_coverage_Ranges(data$x, data$weight, range = range)
    } else {
      valid_x <- is.numeric(data$x) && is.numeric(data$xend)
      if (valid_x) {
        coverage <- compute_coverage_numeric(data$x, data$xend, data$weight,
                                             range = range)
      } else {
...
compute_coverage_Ranges <- function(x, weight = NULL, range = S4Range(x)) {
  ...
  if (inherits(x, "GenomicRanges")) {
    ...
    stop("Don't know how to compute coverage for class ", class(x)[[1]], ".")
  }
