...
ScaleS4Continuous <- ggproto_sibling(
  ...
  get_breaks = function(self, limits = self$get_limits()) {
    ...
    if (S4ZeroRange(limits)) {
      ...
      if (!is.null(self$n.breaks) && ggplot2:::trans_support_nbreaks(self$trans)) {
        breaks <- self$trans$breaks(limits, self$n.breaks)
      } else {
        if (!is.null(self$n.breaks)) {
          rlang::warn("Ignoring n.breaks. Use a trans object that supports setting number of breaks")
        }
  ...
  get_breaks_minor = function(self, n = 2,
    ...
    if (identical(self$minor_breaks, NA)) {
      rlang::abort("Invalid minor_breaks specification. Use NULL, not NA.")
    }
